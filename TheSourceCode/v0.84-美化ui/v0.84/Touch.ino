...
void touch()
{
  ...
  if (p.z < 900 && p.z > 240 && p.x < 1000 && p.x > 60 && p.y < 1000 && p.y > 60)
  {
    ...
    if (coordinates_state == 1) {
      show_string("p.x", x, y, 1, fc1, BLACK, 0);
      show_string_no_fc("p.y", x, y + 8, 1, BLACK, 0);
      show_string_no_fc("p.z", x, y + 16, 1, BLACK, 0);
      show_string_no_fc("px", x + 45, y, 1, BLACK, 0);
      show_string_no_fc("py", x + 45, y + 8, 1, BLACK, 0);
...
void main_touch() //主界面的触摸
{
  ...
  {
    if (px < 189 && px > 129 && py < 130 && py > 90) //加温
    {
      set_temp += 10;
      sleep_count = 0;
      set_temp_limit();
    ...
    else if (px < 189 && px > 129 && py < 190 && py > 150) //减温
    {
      set_temp -= 10;
      sleep_count = 0;
      set_temp_limit();
      ...
      my_lcd.Set_Text_Size(3);
      my_lcd.Set_Text_colour(WHITE);
      if (set_temp < 100 && set_temp > 10) {
        my_lcd.Fill_Rect(parameterX + 40, parameterY + 10, 7, 24, BLACK);
        my_lcd.Print_Number_Int(set_temp, parameterX - 14, parameterY + 10, 4, ' ', 10);
      }
      else if (set_temp >= 100) my_lcd.Print_Number_Int(set_temp, parameterX - 5, parameterY + 10, 4, ' ', 10);
      buzzer(1);
    }
  ...
  //加热总开关
  if (px < 320 && px > 258 && py < 130 + 29 && py > 130 - 5) //开启加热
  {
    t12_switch = 1; //开启总开关
    display_state = 1; //刷新一下
    buzzer(1);
  }
  else if (px < 320 && px > 258 && py < 180 + 29 && py > 180 - 5) //关闭加热
  {
    t12_switch = 0; //关闭总开关
    display_state = 1; //刷新一下
  ...
  //一键设置400度
  else if (px < 83 + 29 && px > 83 - 5 && py < 240 && py > 215)
  {
    set_temp = 400;
    sleep_count = 0;
    buzzer(1);
  ...
  //一键设置320度
  else if (px < 83 + 60 + 29 && px > 83 + 60 - 5 && py < 240 && py > 215)
  {
    set_temp = 320;
    sleep_count = 0;
    buzzer(1);
  ...
  //一键设置240度
  else if (px < 83 + 60 * 2 + 29 && px > 83 + 60 * 2 - 5 && py < 240 && py > 215)
  {
    set_temp = 240;
    sleep_count = 0;
    buzzer(1);
...
void set_touch() //设置界面的触摸
{
  ...
  //进入PID界面
  else if (px < 123 && px > 20 && py < 65 && py > 10)
  {
    buzzer(1);
  ...
  //进入Sleep界面
  else if (px < 309 + 5 && px > 180 && py < 65  && py > 10)
  {
    buzzer(1);
  ...
  //进入屏幕设置界面
  else if (px < 265 && px > 35 && py < 135 && py > 100)
  {
    buzzer(1);
...
void pid_touch() //PID界面的触摸
{
  ...
  else if (px < 80 && px > 0 && py < 55 && py > 0) //改P数值
  {
  ...
  else if (px < 205 && px > 115 && py < 55 && py > 0) //改I数值
  {
    keyboard_state = 1; //调出键盘显示
    keyboard_touch_state = 1; //开启键盘触摸
  ...
  else if (px < 320 && px > 230 && py < 55 && py > 0) //改D数值
  {
    keyboard_state = 1; //调出键盘显示
    keyboard_touch_state = 1; //开启键盘触摸
...
void sleep_touch() //休眠界面的触摸
{
  ...
  else if (px < 120 && px > 0 && py < 65 && py > 20) //改sleep_temp数值
  {
    keyboard_state = 1; //调出键盘显示
    keyboard_touch_state = 1; //开启键盘触摸
  ...
  else if (px < 320 && px > 220 && py < 65 && py > 20) //改sleep_count_set数值
  {
    keyboard_state = 1; //调出键盘显示
    keyboard_touch_state = 1; //开启键盘触摸
