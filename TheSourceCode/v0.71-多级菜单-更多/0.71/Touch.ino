...
void touch()
{
  ...
  pinMode(XM, OUTPUT);
  pinMode(YP, OUTPUT);
  if (p.z < MAX_PZ && p.z > MIN_PZ && p.x < 1500 && p.x > 15)
  {
    /*因为是横屏,所以
       MAX_PX --> py的0
       MIN_PX --> py的240
       ...
       MIN_PY --> px的0
        ------------------------------------
        - MAX_PX             MAX_PX/MAX_PY -
        -                                  -
        -                                  -
        -                                  -
        -                                  -
        - MIN_PX/MIN_PY             MAX_PY -
        ------------------------------------
    */
    ...
    if (millis() - touch_time > 100)//不让键盘按键运行太快
    {
...
void main_touch() //主界面的触摸
{
  ...
  if (millis() - touch_time > 60) //不让调温按键运行太快
  {
    if (px < 320 && px > 250 && py < 45 && py > 0)
    {
      ...
      set_temp_limit();
      buzzer1();
      show_number_int_0(set_temp, 45, 150, 3, WHITE, BLACK  , 0); //刷新设置温度数值
    }
...
void set_touch() //设置界面的触摸
{
  ...
  //进入屏幕设置界面
  else if (px < 320 && px > 0 && py < 40 * 3 && py > 0)
  {
    buzzer1();
...
void screen_set_touch() //屏幕设置界面的触摸
{
  ...
  else if (px < 320 && px > 0 && py < 140 && py > 120) //开关显示坐标
  {
    buzzer1();
